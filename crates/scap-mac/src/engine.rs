...
pub fn create_capturer(
    ...
    let filter = match target {
        ...
        Target::Display(display) => {
            ...
            let sc_display = displays
                .iter()
                .find(|sc_dis| sc_dis.display_id() == display.direct_display_id())
    ...
    let pixel_format = match options.base.output_type {
    ...
    stream_config.set_shows_cursor(options.base.show_cursor);
    stream_config.set_minimum_frame_interval(cm::Time {
        value: 1,
        scale: options.base.fps as i32,
        epoch: 0,
        flags: cm::TimeFlags::VALID,
    });
    stream_config.set_captures_audio(options.base.captures_audio);
    ...
    if options.base.captures_audio {
        stream
...
pub fn get_output_frame_size(options: &MacCaptureOptions) -> [u32; 2] {
    let target = options
        .base
        .target
        .clone()
...
pub fn get_crop_area(options: &MacCaptureOptions) -> Area {
    let target = options
        .base
        .target
        .clone()
                ...
                .unwrap()
            {
                0 => unsafe {
                    return Some(Frame::Video(match output_type {
                        FrameType::YUVFrame => {
                            let yuvframe = super::pixelformat::create_yuv_frame(
                                sample.as_mut(),
                                frame_system_time,
                            )
                            .unwrap();
                        ...
                        FrameType::RGB => {
                            let rgbframe = super::pixelformat::create_rgb_frame(
                                sample.as_mut(),
                                frame_system_time,
                            )
                            .unwrap();
                        ...
                        FrameType::BGR0 => {
                            let bgrframe = super::pixelformat::create_bgr_frame(
                                sample.as_mut(),
                                frame_system_time,
                            )
                            .unwrap();
                        ...
                        FrameType::BGRAFrame => {
                            let bgraframe = super::pixelformat::create_bgra_frame(
                                sample.as_mut(),
                                frame_system_time,
                            )
                            .unwrap();
