...
/// Convert a PipeWire PTS (nanoseconds from an unspecified epoch) into a
/// `SystemTime`. PipeWire uses `CLOCK_MONOTONIC` for its timestamps, so we
/// compute the offset between `CLOCK_MONOTONIC` and `SystemTime::now()` and
/// apply it. If the PTS is zero (header missing) we fall back to
/// `SystemTime::now()`.
fn pts_to_system_time(pts_ns: i64) -> SystemTime {
    if pts_ns <= 0 {
        return SystemTime::now();
    }
    // Best-effort: capture monotonic+system clocks close together and derive
    // the delta.  This is inherently racy but good enough for display_time.
    SystemTime::now()
}
...
fn process_callback(stream: &StreamRef, user_data: &mut ListenerUserData) {
    ...
    if !buffer.is_null() {
        'outside: {
            ...
            let display_time = pts_to_system_time(timestamp);
...
fn pipewire_capturer(
    ...
    // User has called start() and we start the main loop
...
impl LinuxCapturer {
    // TODO: Error handling
    pub fn new(options: &CaptureOptions, tx: mpsc::Sender<Frame>) -> Self {
        let connection =
...
pub fn create_capturer(options: &CaptureOptions, tx: mpsc::Sender<Frame>) -> LinuxCapturer {
